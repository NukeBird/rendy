...
Rendy::AbstractShaderRef Rendy::GAPI::make_shader(ShaderSourceRef source)
{
	OPTICK_EVENT();
	return shader_factory->make(source);
}
...
{
	OPTICK_EVENT();
	return buffer_factory->make(BufferTarget::ElementArray, size, ptr);
}

Rendy::AbstractBufferRef Rendy::GAPI::make_sbo(uint32_t size, const void* ptr)
{
	OPTICK_EVENT();
	return buffer_factory->make(BufferTarget::ShaderStorage, size, ptr);
}

Rendy::AbstractBufferRef Rendy::GAPI::make_cbo(uint32_t size, const void* ptr)
{
	OPTICK_EVENT();
	return buffer_factory->make(BufferTarget::AtomicCounter, size, ptr);
}
